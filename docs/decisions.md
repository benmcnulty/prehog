...
| **Error Tracking** | Declined for MVP | The site has no error-tracking infrastructure anywhere yet, and this page has no server-side logic to fail. Client-side JS errors here would be caught faster by the existing Playwright suite than by a monitoring dashboard. Worth reconsidering only if PostHog error tracking gets adopted site-wide later — this page shouldn't be the one place it exists in isolation. |
